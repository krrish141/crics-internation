(function ($) {
    ...
    $(".pricing-block-one, .project-details__tags li, .sidebar__tags-list a").hover(function () {
    ...
    function ajaxForm() {
        ...
        const forms = document.querySelectorAll(".contact-form2, .contact-form1, .subscribe");
    ...
    let LoadingCounter = setInterval(function () {
        if (percentage <= 100) {
            ...
            let increment = Math.ceil(100 / (loadingDuration / 10));
    ...
    function preLoader() {
        loadingDuration = Date.now() - startTime;
        percentage = 100; // Ensure it reaches 100%
        $(".loading-counter").text(percentage + "%");
